...
| 仅允许本企业钉钉用户登录 | 默认开启；登录回调必须带钉钉企业 corpId，且必须在允许列表内。开启后，非本企业钉钉账号不会进入匹配、绑定或自动创建流程 |
| 允许登录的企业 CorpId | 逗号分隔，例如 `dingxxxxxxxx`。开启“仅允许本企业钉钉用户登录”时必须填写，只有列表内企业用户可以登录和自动创建 |
